...
## 重要な前提
- HPB（ホットペッパービューティー）の価格は全て税込表示です。価格に言及する際は「税込」と記載する必要はありません。
...
| 3-4 | 追加料金・オプション料金が全て明記されており、価格の透明性が高い |
...
ページ内の「予約比率」セクションから以下の情報を取得してください：
...
※ 予約比率のデータが見つからない場合は、gender_ratioとage_ratioはnullを設定してください。
...
また、男女比・年齢層データを踏まえた「ターゲット顧客層に対する改善提案」も含めてください。
"""
